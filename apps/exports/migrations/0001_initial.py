...
def create_tables_if_not_exist(apps, schema_editor):
    """Create export tables only if they don't already exist.
    Handles the case where tables were created before migrations were added.
    """
    ...
    # Create in dependency order: ExportTemplate before ExportHistory and ReportSchedule
    model_names = [
        'ExportTemplate',   # no export deps
        'ExportHistory',    # depends on ExportTemplate, Project, AUTH_USER_MODEL
        'ReportSchedule',   # depends on ExportTemplate, Project, AUTH_USER_MODEL
    ]
...
def drop_tables(apps, schema_editor):
    """Reverse migration: drop export tables in reverse dependency order."""
    model_names = ['ReportSchedule', 'ExportHistory', 'ExportTemplate']
    for model_name in model_names:
        model = apps.get_model('exports', model_name)
